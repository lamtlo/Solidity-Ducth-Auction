...
def test_update_price(auction):
    ...
    assert new_price in range(initial_price - discountRate * duration, initial_price - discountRate * (duration + 5), -discountRate)
...
def test_bid_transfer_eth(auction, seller, bidder):
    ...
    assert seller.balance() in range(balance_before_seller + current_price, balance_before_seller + current_price - discountRate * 5, -discountRate)
    assert bidder.balance() in range(balance_before_bidder - current_price, balance_before_seller - current_price + discountRate * 5, discountRate)
